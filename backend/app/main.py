...
async def lifespan(app: FastAPI):
    ...
    try:
        if os.path.exists(model_path) and os.path.exists(scaler_path):
            ...
            print("⚠️ Warning: Model artifacts not found. Please run Mage pipeline first.")
...
def predict_cluster(features: ProvinceFeatures):
    if "kmeans" not in ml_models or "scaler" not in ml_models:
        raise HTTPException(status_code=503, detail="Model belum siap. Jalankan pipeline training dulu.")
